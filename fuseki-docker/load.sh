...
if [ $# -eq 0 ] ; then
  echo "$0 [DB] [PATTERN ...]"
  echo "Load one or more RDF files into Jena Fuseki TDB2 database DB."
  echo ""
  ...
  echo "options to pass to tdbloader2, e.g. --graph=https://example.org/graph#name"
  exit 0
fi
...
echo "#########"
echo "Loading to Fuseki TDB2 database $DB:"
echo ""
echo $files
echo "#########"
