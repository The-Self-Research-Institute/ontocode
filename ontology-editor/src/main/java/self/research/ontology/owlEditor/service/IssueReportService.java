...
public class IssueReportService {
    ...
    public IssueReportService(IssueReportRepository issueReportRepository, OpenProjectService openProjectService) {
    ...
    public IssueReportResult submitIssueReport(IssueReport issueReport, List<MultipartFile> attachments) {
        try {
            ...
            String fullDescription = buildTrackerDescription(issueReport);

            String priority = (issueReport.getPriority() != null && !issueReport.getPriority().isEmpty())
                ? issueReport.getPriority()
                : OpenProjectService.determinePriority(issueReport.getTitle(), issueReport.getDescription());

            String issueType = (issueReport.getIssueType() != null && !issueReport.getIssueType().isEmpty())
                ? issueReport.getIssueType()
                : "Task";

            if (openProjectService.isEnabled()) {
                try {
                    OpenProjectService.OpenProjectIssueResult trackerResult = openProjectService.createIssue(
                    ...
                    if (trackerResult.isSuccess()) {
                        issueReport.setIssueKey(trackerResult.getIssueKey());
                        issueReport.setIssueUrl(trackerResult.getIssueUrl());
                        ...
                        if (attachments != null && !attachments.isEmpty()) {
                            ...
                            for (MultipartFile file : attachments) {
                                try {
                                    boolean uploaded = openProjectService.uploadAttachment(
                                        trackerResult.getIssueKey(),
                        ...
                        log.info("Issue report submitted successfully: {} -> {}", saved.getId(), trackerResult.getIssueKey());
                        ...
                        return IssueReportResult.builder()
                           ...
                           .message(
                            ...
                            .issueKey(trackerResult.getIssueKey())
                            .issueUrl(trackerResult.getIssueUrl())
                            .build();
                    } else {
                        issueReport.setStatus(IssueReport.IssueStatus.FAILED);
                        issueReport.setFailureReason(trackerResult.getErrorMessage());
                        IssueReport saved = issueReportRepository.save(issueReport);

                        log.warn("OpenProject issue creation failed, saved locally: {}", saved.getId());

                        return IssueReportResult.builder()
                            .success(true)
                            .message(
                                 "Task".equalsIgnoreCase(issueType)
                                     ? "Feature request saved locally but failed to create OpenProject ticket. Our team has been notified."
                                     : "Issue saved locally but failed to create OpenProject ticket. Our team has been notified."
                            )
                            .issueReportId(saved.getId())
                            .trackerFailureReason(trackerResult.getErrorMessage())
                            .build();
                    }

                } catch (Exception e) {
                    log.error("Exception while creating OpenProject issue", e);
                    issueReport.setStatus(IssueReport.IssueStatus.FAILED);
                    ...
                    return IssueReportResult.builder()
                        .success(true)
                         .message(
                             "Task".equalsIgnoreCase(issueType)
                                 ? "Feature request saved locally but failed to create OpenProject ticket: " + e.getMessage()
                                 : "Issue saved locally but failed to create OpenProject ticket: " + e.getMessage()
                               )
                        .issueReportId(saved.getId())
                        .trackerFailureReason(e.getMessage())
                        .build();
                }
            } else {
                ...
                log.info("Issue report saved locally (OpenProject disabled): {}", saved.getId());
    ...
    private String buildTrackerDescription(IssueReport issueReport) {
    ...
    public OpenProjectService.OpenProjectValidationResult validateOpenProjectConnection() {
        return openProjectService.validateConnection();
    }
