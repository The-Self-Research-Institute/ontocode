...
public class GraphViewController {
    ...
    public ResponseEntity<Map<String, Object>> getInitialGraph(
        ...
        try {
            log.info("Fetching initial graph for project: {} (max {} nodes)", projectId, maxNodes);
            ...
            List<GraphGeneratingService.Node> nodes = graph.getNodes();
            List<GraphGeneratingService.Edge> edges = graph.getEdges();

            // Limit to maxNodes
            if (nodes.size() > maxNodes) {
                nodes = nodes.subList(0, maxNodes);
                Set<String> nodeIds = nodes.stream().map(GraphGeneratingService.Node::getId).collect(Collectors.toSet());
                edges = edges.stream()
    ...
    public ResponseEntity<Map<String, Object>> expandNode(
        ...
        try {
            log.info("Expanding node: {} in project: {}", nodeId, projectId);
