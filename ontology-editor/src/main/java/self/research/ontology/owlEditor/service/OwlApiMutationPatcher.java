...
public class OwlApiMutationPatcher {
    ...
    private boolean createAnnotationProperty(OWLDataFactory df,
        ...
        if (op.parent() != null && !op.parent().isBlank()) {
            toAdd.add(df.getOWLSubAnnotationPropertyOfAxiom(
                    prop, df.getOWLAnnotationProperty(IRI.create(op.parent()))));
        }
        return true;
    }
    ...
    private boolean addSubPropertyOf(OWLOntology ontology, OWLDataFactory df, String subIri, String superIri, Set<OWLAxiom> toAdd) {
        ...
        toAdd.add(df.getOWLSubObjectPropertyOfAxiom(
                df.getOWLObjectProperty(sub),
                df.getOWLObjectProperty(sup)));
        return true;
    }
    ...
    private boolean removeSubPropertyOf(OWLOntology ontology, OWLDataFactory df, String subIri, String superIri, Set<OWLAxiom> toRemove) {
        ...
        IRI sup = IRI.create(superIri);
        if (ontology.containsAnnotationPropertyInSignature(sub, Imports.EXCLUDED)) {
            toRemove.add(df.getOWLSubAnnotationPropertyOfAxiom(
                df.getOWLAnnotationProperty(sub), df.getOWLAnnotationProperty(sup)));
            return true;
        }
        if (ontology.containsDataPropertyInSignature(sub, Imports.EXCLUDED)) {
            toRemove.add(df.getOWLSubDataPropertyOfAxiom(
                df.getOWLDataProperty(sub), df.getOWLDataProperty(sup)));
            return true;
        }
        if (superIri.contains("XMLSchema#") || subIri.contains("XMLSchema#")) {
