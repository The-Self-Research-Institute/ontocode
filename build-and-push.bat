...
@REM docker buildx build --no-cache --platform linux/amd64,linux/arm64 -t %REGISTRY%/ontocode-editor:%VERSION% -f Dockerfile.editor --push .
@REM if errorlevel 1 goto :error
...
@REM echo [8/8] Building ontocode-web (webapp with HTTPS config)...
@REM docker buildx build --no-cache --platform linux/amd64,linux/arm64 -t %REGISTRY%/ontocode-web:%VERSION% -f Dockerfile.webapp --push .
@REM if errorlevel 1 goto :error
