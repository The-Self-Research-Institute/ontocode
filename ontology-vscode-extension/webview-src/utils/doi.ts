/**
 * DOI (Digital Object Identifier) helpers for the citation picker.
 *
 * Spec references:
 *  - DOI Handbook §2.2 (https://doi.org/10.1000/182): a DOI is a string
 *    "10.<registrant>/<suffix>" where the suffix is opaque and may contain
 *    almost any printable character (slashes, dots, parentheses, ...).
 *  - Crossref / DataCite content negotiation (RFC 7231) — used by the
 *    Java backend to actually resolve a DOI and check existence.
 *
 * Pattern checks alone are NOT validation — they merely confirm syntactic
 * shape. Authoritative validation requires resolving the DOI through
 * `https://doi.org/<doi>` (we do this server-side in CitationService.java).
 */
...
/** Cheap offline shape check. Use {@link validateDoiOnline} for real validation. */
 ...
 * exporter spelled the field. Zotero's own schema uses {@code DOI}
 * (uppercase) for journal articles, books, theses, etc.; older exports
 * and translators sometimes emit lowercase {@code doi}, and many users
 * stash a DOI in the multi-line {@code extra} field as
 * {@code "DOI: 10.xxxx/yyyy"}.
 *
