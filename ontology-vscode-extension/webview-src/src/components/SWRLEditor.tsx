...
import type { SWRLRule, ValidationResult, SQWRLResult } from '../types';
...
const SQWRLQueryPanel: React.FC<{ projectId: string }> = ({ projectId }) => {
    ...
    const executeQuery = useCallback(async () => {
        setIsLoading(true);
        try {
            const response = await apiClient.post('/api/sqwrl/query', { projectId, query });
                    ...
                    {results ? (
                        <table className="min-w-full text-sm">
                            <thead className="bg-gray-100">
                                <tr>
                                    {results.columns.map(col => <th key={col} className="p-2 text-left font-semibold text-gray-600">{col}</th>)}
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-200">
                                {results.rows.map((row, rowIndex) => (
                                    <tr key={rowIndex} className="hover:bg-gray-50">
                                        {results.columns.map(col => <td key={col} className="p-2 text-gray-700 whitespace-nowrap">{row[col]}</td>)}
                                    </tr>
                                ))}
                            </tbody>
                        </table>
...
const SWRLEditor: React.FC<SWRLEditorProps> = ({ projectId, context }) => {
    ...
    const fetchRules = useCallback(async () => {
        try {
            const response = await apiClient.get(`/api/swrl/rules?projectId=${projectId}`);
            setRules(response.data);
        } catch (error) {
            context.notificationService.error('Failed to fetch SWRL rules.', error);
        }
    ...
    useEffect(() => {
        const validate = async () => {
            ...
            try {
                const response = await apiClient.post('/api/swrl/validate', { ruleText: debouncedRuleText });
    ...
    const handleSaveRule = async () => {
        ...
        try {
            if (selectedRule) {
                ...
                await apiClient.post(`/api/swrl/rules`, { ...ruleData, id: selectedRule.id });
                context.notificationService.success(`Rule "${ruleName}" updated.`);
            } else {
                await apiClient.post('/api/swrl/rules', ruleData);
                context.notificationService.success(`Rule "${ruleName}" created.`);
            }
            fetchRules();
        } catch (error) {
            context.notificationService.error("Failed to save rule.", error);
        }
    };
    ...
    const handleDeleteRule = async () => {
        ...
        if (window.confirm(`Are you sure you want to delete the rule "${selectedRule.name}"?`)) {
            try {
                await apiClient.delete(`/api/swrl/rules/${selectedRule.id}`);
                context.notificationService.success(`Rule "${selectedRule.name}" deleted.`);
                handleNewRule(); // Reset view to new rule state
                fetchRules(); // Refresh the list
            } catch (error) {
                context.notificationService.error("Failed to delete rule.", error);
            }
        }
    };

    const handleRunReasoner = () => {
        context.notificationService.info('Running reasoner... In a real app, inferred axioms would be calculated and displayed.');
    };
                ...
                <aside className="w-80 bg-white border-r border-gray-200 flex flex-col">
                    <div className="p-2 border-b border-gray-200 flex gap-2">
                        <button onClick={handleNewRule} className="flex-1 flex items-center justify-center gap-2 text-xs text-white bg-purple-600 hover:bg-purple-700 px-3 py-2 rounded-md transition-colors shadow-sm">
                         ...
                         <button onClick={handleRunReasoner} className="flex-1 flex items-center justify-center gap-2 text-xs text-gray-700 bg-gray-100 hover:bg-gray-200 px-3 py-2 rounded-md transition-colors">
                            <BrainCircuit size={14} /> Run Reasoner
                        </button>
                    </div>
