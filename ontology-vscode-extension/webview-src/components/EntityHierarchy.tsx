...
const EntityHierarchy: React.FC<EntityHierarchyProps> = ({
  ...
  const renderItem = (item: SelectableItem, level = 0): React.JSX.Element => {
    ...
    if (entitiesTab === 'Classes' && !isTreeNode) itemType = 'Classes'; // Fallback for search results

    switch (itemType) {
        // Fix: Replaced undefined 'Box' icon with 'Package' for consistency.
      ...
      <div className={`text-xs font-semibold p-1.5 flex items-center justify-between border-b`}>
        <span>{currentTabConfig?.label} hierarchy</span>
         <div className="flex items-center gap-1">
              {entitiesTab === 'Classes' && (
                 <>
                 <button title="Add subclass" disabled={!selectedItem} onClick={() => onAddItem('subclass')} className="p-0.5 hover:bg-black/20 rounded disabled:opacity-30">
                      <PlusCircle size={14} />
                 </button>
                 <button title="Add sibling class" disabled={!selectedItem} onClick={() => onAddItem('sibling')} className="p-0.5 hover:bg-black/20 rounded disabled:opacity-30">
                      <Binary size={14} />
                 </button>
                 </>
              )}
              {entitiesTab === 'Individuals' && (
                   <button title="Add individual" onClick={() => onAddItem('individual')} className="p-0.5 hover:bg-black/20 rounded">
                      <PlusCircle size={14} />
                 </button>
              )}
             <button title="Delete selected entity" disabled={!selectedItem} onClick={onDeleteItem} className="p-0.5 hover:bg-black/20 rounded disabled:opacity-30">
                <Trash2 size={14} />
             </button>
        </div>
      </div>
