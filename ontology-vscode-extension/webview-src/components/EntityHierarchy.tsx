...
const EntityHierarchy = ({
  ...
  const handleResizeMouseDown = (e: React.MouseEvent) => {
    ...
    const onMouseMove = (moveEvent: MouseEvent) => {
      const next = Math.min(RESIZE_MAX, Math.max(RESIZE_MIN, startWidth + (moveEvent.clientX - startX)));
  ...
  const renderRow = (item: SelectableItem, level = 0): React.JSX.Element => {
    ...
    return (
      <div
        ...
        className={`flex items-center px-2 py-0.5 rounded cursor-pointer transition-all ${isSelected ? "selected" : "hover-overlay"
      ...
      >
        ...
        {multiSelectMode && (
          <div
            className={`w-4 h-4 mr-2 rounded border flex items-center justify-center flex-shrink-0 ${selectedProperties.includes(item.id)
        ...
        {renamingItemId === item.id ? (
          ...
          <div className="flex items-center gap-1 overflow-hidden">
            <span
              className={`text-xs select-none truncate min-w-0 shrink ${isSelected ? "font-semibold" : ""} ${(item as any).isUnsatisfiable ? "text-red-600 font-bold" : ""
                }`}
  ...
  return (
    <aside
      className={`${hideToolbarActions ? 'w-full' : 'w-full md:w-[var(--panel-width)] relative flex-shrink-0'} ${hideToolbarActions ? '' : 'border-r'} flex flex-col h-full`}
      style={{
        backgroundColor: 'var(--color-surface)',
        borderColor: 'var(--color-border)',
        ...(!hideToolbarActions ? { ['--panel-width' as any]: `${panelWidth}px` } : {}),
      }}
    >
            ...
            {(entitiesTab === 'Classes' || entitiesTab === 'ObjectProperties' || entitiesTab === 'DataProperties' || entitiesTab === 'Datatypes' || entitiesTab === 'AnnotationProperties' || entitiesTab === 'Individuals') && (
              <div className="flex items-center gap-0.5 bg-gray-100 rounded px-1 py-0.5">
                <button
                  ...
                  className={`px-2 py-0.5 text-[10px] rounded transition-colors whitespace-nowrap ${viewMode === 'asserted'
                ...
                <button
                  ...
                  className={`px-2 py-0.5 text-[10px] rounded transition-colors whitespace-nowrap ${viewMode === 'inferred'
      ...
      {!hideToolbarActions && (
        <div
          ...
          className={`hidden md:block absolute top-0 right-0 h-full w-1.5 -mr-[3px] cursor-col-resize z-10 transition-colors ${isResizingPanel ? 'bg-purple-500/70' : 'hover:bg-purple-400/40'
            }`}
