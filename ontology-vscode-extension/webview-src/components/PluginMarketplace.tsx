...
export const PluginMarketplace: React.FC<PluginMarketplaceProps> = ({
    ...
    try {
      ...
      const pluginsWithStats = await Promise.all(
        pluginsList.map(async (p: any) => {
          ...
          try {
            const statsResponse = await fetch(`${apiBaseUrl}/api/plugins/${p.pluginId}/stats`, { headers });
            if (statsResponse.ok) {
          ...
          const hasUpdate = isInstalled && installedVersion ? isNewerVersion(p.latestVersion, installedVersion) : false;
  ...
  const refreshPluginStats = async (pluginId: string) => {
    try {
      ...
      const apiBaseUrl = getPluginApiBaseUrl();
      const statsResponse = await fetch(`${apiBaseUrl}/api/plugins/${pluginId}/stats`, { headers });

      if (statsResponse.ok) {
        const stats = await statsResponse.json();

        setPlugins((prev) => prev.map((p) => (p.pluginId === pluginId ? { ...p, stats } : p)));
      }
    } catch (error) {
  ...
  const handleUpdate = async (pluginId: string) => {
    ...
    try {
      ...
      setPlugins((prev) =>
        prev.map((p) => (p.pluginId === pluginId ? { ...p, installedVersion: p.version, hasUpdate: false } : p)),
      );
  ...
  const confirmUninstall = async () => {
    ...
    try {
      ...
      setPlugins((prev) => prev.map((p) => (p.pluginId === uninstallPluginId ? { ...p, installed: false } : p)));
  ...
  const renderStars = (rating: number, totalRatings: number) => {
    return (
      <div className="flex items-center gap-1">
        ...
        <span className="text-xs text-gray-600 ml-1">{rating > 0 ? rating.toFixed(1) : "—"}</span>
        <span className="text-xs text-gray-700">({totalRatings})</span>
      </div>
    );
  };
          ...
          {loading ? (
            ...
            <div className="flex items-center justify-center h-64">
              <div className="text-center">
                ...
                <p className="text-gray-700 text-sm mt-2">Try adjusting your search or filters</p>
              </div>
            </div>
          ) : (
            /* Plugin Grid */
                  ...
                  <div className="text-xs text-gray-700 mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      {plugin.installed && plugin.installedVersion && (
                        <span className="text-gray-500">Installed: v{plugin.installedVersion}</span>
                      )}
                      {plugin.hasUpdate && <span className="text-green-600 font-semibold">→ v{plugin.version}</span>}
                      {!plugin.installed && <span>Latest: v{plugin.version}</span>}
                      {plugin.installed && !plugin.hasUpdate && <span className="text-green-600">✓ Up to date</span>}
                    </div>
                  </div>
      ...
      {showUninstallConfirm && uninstallPluginId && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[60]">
          <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold text-gray-900 mb-3">Confirm Uninstall</h3>
            <p className="text-gray-600 mb-6">
              Are you sure you want to uninstall{" "}
              <span className="font-semibold">{plugins.find((p) => p.pluginId === uninstallPluginId)?.name}</span>? This
              will remove the plugin and its tab from the interface.
            </p>
