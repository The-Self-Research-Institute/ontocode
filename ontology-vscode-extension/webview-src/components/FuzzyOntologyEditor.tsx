...
interface FuzzyOntologyEditorProps {
  projectId: string;
}

const FuzzyOntologyEditor: React.FC<FuzzyOntologyEditorProps> = ({ projectId }) => {
  ...
  const loadFuzzyData = async () => {
    try {
      ...
      const response = await fetch(`${window.API_BASE_URL}/api/projects/${projectId}/query`, {
        method: 'POST',
        headers: {
  ...
  const saveFuzzyOntology = async () => {
    try {
      ...
      const response = await fetch(`${window.API_BASE_URL}/api/projects/${projectId}/update`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('authToken')}`
            ...
            <div className="bg-[#252525] p-4 rounded border border-gray-700">
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full px-3 py-2 bg-[#1e1e1e] border border-gray-600 rounded text-black"
                  style={{ color: 'black' }}
                />
                <textarea
                  ...
                  className="w-full px-3 py-2 bg-[#1e1e1e] border border-gray-600 rounded h-20 text-black"
                  style={{ color: 'black' }}
                />
                <textarea
...
ORDER BY DESC(?degree)`}
                className="w-full px-3 py-2 bg-[#1e1e1e] border border-gray-600 rounded h-64 font-mono text-sm text-black"
                style={{ color: 'black' }}
              />
              <button
...
export default FuzzyOntologyEditor;
