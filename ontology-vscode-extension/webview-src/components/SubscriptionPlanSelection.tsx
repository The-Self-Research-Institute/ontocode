...
const SubscriptionPlanSelection: React.FC<SubscriptionPlanSelectionProps> = ({
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 relative overflow-y-auto">
            {/* Animated background */}
            ...
            <div className="relative z-10 min-h-screen overflow-y-auto p-6">
                ...
                <div className="flex items-center justify-between mb-4">
                    ...
                    <button
                        onClick={onLogout}
                        className="flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white rounded-lg transition-all"
                    >
                        <LogOut size={18} />
                        Logout
                    </button>
                ...
                <div className="flex items-center justify-center py-4">
                    <div className="grid md:grid-cols-3 gap-4 max-w-7xl w-full">
                        {plans.map((plan) => (
                            <div
                                key={plan.id}
                                onClick={() => setSelectedPlan(plan.id)}
                                className={`
                                    relative bg-white/10 backdrop-blur-xl rounded-2xl p-5 cursor-pointer transition-all duration-300 max-h-[600px] flex flex-col
                                    border-2 ${selectedPlan === plan.id 
                            ...
                            >
                                {plan.badge && (
                                    <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
                                        <span className={`bg-gradient-to-r ${plan.gradient} text-white px-3 py-1 rounded-full text-xs font-bold shadow-lg`}>
                                            {plan.badge}
                                        </span>
                                    </div>
                                )}
                                ...
                                {/* Icon & Title */}
                                <div className="mb-3">
                                    <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${plan.gradient} flex items-center justify-center text-white mb-2 shadow-lg`}>
                ...
                {/* Action Buttons */}
                <div className="relative z-20 flex items-center justify-center gap-4 mt-4 mb-3">
                    <button
                        onClick={onSkip}
