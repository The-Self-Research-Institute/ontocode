 ...
 * 
 * Based on Protégé's design for data property ranges:
 * - "Built in datatypes" tab: Shows list of XSD/OWL/RDF datatypes
 * - "Data range expression" tab: For complex data range expressions
 */
...
// Built-in datatypes matching Protégé's list
...
const DataPropertyRangeDialog: React.FC<DataPropertyRangeDialogProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'built-in' | 'expression'>('built-in');
  const [selectedDatatype, setSelectedDatatype] = useState<string | null>(null);
  const [expression, setExpression] = useState('');

  // Merge provided datatypes with built-in ones
  const allDatatypes = [
    ...BUILT_IN_DATATYPES,
  ...
  const isValidSelection = activeTab === 'built-in' 
              ...
              {allDatatypes.map(dt => (
                ...
                >
                  <span className={`w-3 h-3 rounded-full flex-shrink-0 ${
                    selectedDatatype === dt.id ? 'bg-red-600' : 'bg-red-500'
                  }`} />
