...
// Adjust accent brightness for light vs dark mode
function adjustAccentForMode(hex: string, mode: 'light' | 'dark'): string {
  ...
  if (mode === 'dark') {
    // Make brighter for dark mode (like "400" shade)
    l = Math.min(0.75, l + 0.15);
  } else {
    // Make slightly deeper for light mode (like "600" shade)
...
export const ThemeProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  // Determine actual mode (resolve 'auto' to 'light' or 'dark')
  ...
  useEffect(() => {
    ...
    // Clean up old single-color storage (migration)
  ...
  useEffect(() => {
    ...
    // Apply accent and computed on-accent
  ...
  const updateColors = (colors: Partial<ThemeColors>) => {
    // Update colors for the current actual mode
    if (actualMode === 'dark') {
  ...
  const resetToDefault = () => {
    // Reset colors for the current actual mode
    if (actualMode === 'dark') {
      setCustomDarkColors({});
