...
branch_web_remote_build() {
  ...
  local rsync_paths=(pom.xml checkstyle.xml checkstyle-suppressions.xml shared/ \
...
build_desktop() {
  local platform="$1" update_host="$2" api_base="$3"
  local webapp_env="$ROOT/ontology-vscode-extension/webview-src/.env.production"
  {
    echo "# Generated by deploy-coretopia-release.sh — do not edit by hand"
    echo "VITE_CLOUD_GATEWAY_URL=$api_base"
    echo "VITE_CLOUD_EDITOR_URL=$api_base"
    echo "VITE_CLOUD_PLUGIN_URL=$api_base:8087"
  } > "$webapp_env"
  echo "[progress][desktop] regenerated webview .env.production → $api_base"
  (
    flock -x 9
...
build_desktop_win_via_windows_host() {
  local update_host="$1" api_base="$2"
...
branch_desktop_windows() {
  ...
  case "$host_platform" in
    win)
      build_desktop "win" "$update_host" "$api_base" || { echo "ERROR: windows build failed" >&2; return 1; }
      ;;
    linux)
      if is_wsl; then
        build_desktop_win_via_windows_host "$update_host" "$api_base" || { echo "ERROR: windows build via WSL cross-call to the Windows host failed" >&2; return 1; }
      else
...
branch_desktop_linux_remote_build() {
  ...
  if [[ $UPLOAD_ONLY_ARG -eq 1 ]]; then
    ...
    local desktop_webapp_env="$ROOT/ontology-vscode-extension/webview-src/.env.production"
    {
      echo "# Generated by deploy-coretopia-release.sh --mode $m — do not edit by hand"
      echo "VITE_CLOUD_GATEWAY_URL=$api_base"
      echo "VITE_CLOUD_EDITOR_URL=$api_base"
    ...
    echo "[progress][$m-linux] regenerated webview .env.production → $api_base"
...
branch_desktop_mac() {
  ...
  build_desktop "mac" "$update_host" "$api_base" || { echo "ERROR: mac build failed" >&2; return 1; }
